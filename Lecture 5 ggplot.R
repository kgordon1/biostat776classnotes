...
library("tidyverse")
library("here")
maacs <- read_csv(here("jhustatcomputing-main", "data", "bmi_pm25_no2_sim.csv"),
                  col_types = "nnci"
)
maacs
...
## ------------------------------------------------------------------------------------------------------------------------
#aes starts with what is the explanatory variable is x, outcome is y
g <- ggplot(maacs, aes(
  x = logpm25,
  y = NocturnalSympt
))
...
## ------------------------------------------------------------------------------------------------------------------------
#Adding more layers
#smooth Because the data appear rather noisy, it might be better if we added a smoother on top of the points to see if there is a trend in the data with PM2.5.
...
#The default smoother is a loess smoother, which is flexible and nonparametric
#but might be too flexible for our purposes.

## ------------------------------------------------------------------------------------------------------------------------
#Perhaps we’d prefer a simple linear regression line
#to highlight any first order trends.
#We can do this by specifying method = "lm" to geom_smooth().
#| fig-cap: "Scatterplot with linear regression line"
#| message: false
g +
  geom_point() +
  geom_smooth(method = "lm")
#darkgrey shading is confidence interval
#Here, we can see there appears to be a slight increasing trend,
#suggesting that higher levels of PM2.5 are associated with
#increased days with nocturnal symptoms.
...
#Let’s use the ggplot() function in another dataset, namely the palmerpenguins dataset. These data contain observations for 344 penguins. There are 3 different species of penguins in this dataset, collected from 3 islands in the Palmer Archipelago, Antarctica.

glimpse(penguins)

penguins %>%
  ggplot(mapping = aes(x = "flipper_length_m", y =
"bill_length_mm", color "species")) +
geom_point() +
